...
export default {
  ...
  methods: {
    ...
    addToCart(product) {
      ...
      if (!userJson) {
        ...
        if (product.quality > 0) {
          ...
          this.updateProductQuantity(product.id, product.quality - 1);
          ...
          .then((response) => {
            console.log('Cart API response:', response.data); // Add this for debugging
    ...
    async updateProductQuantity(productId, newQuantity) {
      try {
        await axios.patch(`http://localhost:3000/api/products/${productId}`, { quality: newQuantity });
      } catch (error) {
